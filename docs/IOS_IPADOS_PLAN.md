...
project is required. **Decision: XcodeGen.** A committed `project.yml` defines
the iOS target; `xcodegen` generates `MonocleRadio.xcodeproj` locally (the
generated project and Info.plist are gitignored). This keeps the repo
diff-friendly and lets the project be authored without a Mac. The existing
`Makefile`/SPM flow is untouched and remains how the Mac app builds.
...
1. Commit `project.yml` (XcodeGen) defining the `MonocleRadioiOS` target,
   which depends on the local `MonocleRadioKit` package.
2. iOS entry point: `AVAudioSession` category setup, instantiate
   `RadioViewModel`, root view. Info.plist (generated by XcodeGen) with
   `UIBackgroundModes: audio`. iOS app icon rendered from
   `assets/monocle-logo.svg`.
